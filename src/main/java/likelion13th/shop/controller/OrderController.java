...
public class OrderController {
    ...
    public ApiResponse<?> createOrder(
        ...
        try {
            ...
            log.error("X [ERROR] 주문 생성 중 예외 발생: {}",e.getReason().getMessage());
            throw e;
        } catch (Exception e){
            log.error("X [ERROR] 알 수 없는 예외 발생: {}",e.getMessage());
    ...
    public ApiResponse<?> deleteOrderById(@PathVariable Long orderId) {
        ...
        try {
            ...
            log.error("X [ERROR] 개별 주문 조회 중 예외 발생: {}", e.getReason().getMessage());
            throw e;
        } catch (Exception e){
            log.error("X [ERROR] 알 수 없는 예외 발생: {}",e.getMessage());
            throw new GeneralException(ErrorCode.INTERNAL_SERVER_ERROR);
    ...
    public ApiResponse<?> cancelOrder(@PathVariable Long orderId) {
        ...
        try {
            ...
            log.error("X [ERROR] 주문 취소 중 예외 발생: {}", e.getReason().getMessage());
            throw e;
        } catch (Exception e) {
            log.error("X [ERROR] 일 수 없는 예외 발생: {}", e.getMessage());
            throw new GeneralException(ErrorCode.INTERNAL_SERVER_ERROR);
        }
    }
}
